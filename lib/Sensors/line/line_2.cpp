...
void lisenses::init()
{
    ...
    for (short ili = 0; ili < 32; ili ++){
        ...
        }else{line_sensors[ili].vector.lenght = all_len[2];}
    }
}
    ...
    for (int c = 0; c < 8; c++)
    {
        ...
        for (int step = 0; step < 4; step++)
        {
            ...
            if (line_sensors[step * 8 + c].bool_value){
                livectors.push_back(line_sensors[step * 8 + c].vector);
            }
            // for (int i = 0; i < 32; i++)
            // {
...
line lisenses::get_line()
{
    ...
    for (int i = 0; i < ret_i; i++){
        ...
        lisin += sin(livectors[i].angle) * 100 * livectors[i].lenght;
        licos += cos(livectors[i].angle) * 100 * livectors[i].lenght;
    }
     Serial.print(lisin);
    Serial.print(" ");
    Serial.print(licos);
    ...
    ret.lenght = sqrt(lisin*lisin + licos*licos)/livectors.size();
